...
void Base()
{
	...
	{
	case Z_AXIS:
		glRotatef(g_Ball.rotation.z, 0, 0, 1);
		break;
	case X_AXIS:
		glRotatef(g_Ball.rotation.x, 1, 0, 0);
		break;
	case Y_AXIS:
		glRotatef(g_Ball.rotation.y, 0, 1, 0);
		break;
	}
...
void Keyboard(unsigned char key, int x, int y)
{
	...
	if (key == 'i')
	{
		...
		g_WorldDegree.y = 0;
		g_WorldDegree.z = 0;
		g_Ball.rotation = { 0,0,0 };
		g_Ball.position = { 0,0,0 };
		glLoadIdentity();
	}
	if (key == 'd')
	{
	...
	if (key == 'n')
	{
		g_ballRotation = Z_AXIS;
		g_Ball.rotation.x = 0;
		g_Ball.rotation.z += 10;
		g_Ball.position.x -= 0.1;
	}
	if (key == 'N')
	{
		g_ballRotation = Z_AXIS;
	...
	if (key == 'm')
	{
		g_ballRotation = Y_AXIS;
		g_Ball.rotation.x = 0;
		g_Ball.rotation.z = 0;
		g_Ball.rotation.y += 30;
	}
	if (key == 'M')
	{
	...
	if (key == 'l')
	{
		g_ballRotation = X_AXIS;
		g_Ball.rotation.z = 0;
		g_Ball.rotation.x -= 10;
		g_Ball.position.z -= 0.1;
	}
	if (key == 'L')
	{
		g_ballRotation = X_AXIS;
