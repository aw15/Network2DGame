...
#define BOUNDARY_MAX_X  650.f
#define BOUNDARY_MIN_X  150.f
#define BOUNDARY_MAX_Y  550.f
#define BOUNDARY_MIN_Y  50.f
...
void Lesson10()
{
	Object boundary({ 400, 300,0 }, { 1.0,0.0,0.0,1.0 }, 500);
	boundary.Render(LINE);

	for (list<Object*>::iterator iter = g_vObjects.begin(); iter != g_vObjects.end();)
	{
		Object* object = *iter;
		//object->setsize(1);
		object->Move();
		...
		if (current.x < BOUNDARY_MIN_X|| current.x > BOUNDARY_MAX_X ||current.y < BOUNDARY_MIN_Y|| current.y > BOUNDARY_MAX_Y)
		{
			object->Release();
			delete (object);
			iter = g_vObjects.erase(iter);
		}
		else
		{
			object->Render(RECT);
...
void Mouse(int button, int state, int x, int y)//x,y값은 윈도우 기준이기 때문에 좌측상단이 0,0이다.
{
	...
	if (button == GLUT_LEFT_BUTTON && state == GLUT_DOWN)
	{
		...
		newObject->SetSpeed(15.f);
		float x = rand() / (float)RAND_MAX;
		float y = rand() / (float)RAND_MAX;
		rand() % 2 == 1 ? x = x*-1 : x;
		rand() % 2 == 1 ? y = y*-1 : y;
		Transform dir = { x , y , 0.0f };
		newObject->SetCurrentDir(dir);
...
void Keyboard(unsigned char key, int x, int y)
{
	if (key == 'w')
	{
		for (list<Object*>::iterator iter = g_vObjects.begin(); iter != g_vObjects.end(); ++iter)
		{
			Object* object = *iter;
			object->SetSpeed(5);
		}
