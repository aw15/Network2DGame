...
void Lesson8()
{
	...
	if (g_target != nullptr)
	{
		g_target->Render(RECT);
		targetPosition = g_target->GetTransform();
	}
	for (list<Object*>::iterator iter = vObjects.begin(); iter != vObjects.end(); ++iter)
	{
		...
		if (g_start)
		{
			...
			Transform resultPos = targetPosition - currentPos;
			cout << resultPos.x<<" " << resultPos.y<<endl;
			object->SetSpeed(0.03f);

			object->Move(resultPos);
		}
...
void Mouse(int button, int state, int x, int y)//x,y값은 윈도우 기준이기 때문에 좌측상단이 0,0이다.
{
	...
	if (button == GLUT_RIGHT_BUTTON && state == GLUT_DOWN)
	{
		...
		Object* newObject = new Object({ float(x),float(y),0.0f }, 20);
	...
	else if (button == GLUT_LEFT_BUTTON&&state == GLUT_DOWN)
	{
		...
		y = 600 - y;
		g_target = new Object({ float(x),float(y),0.0f }, 10);
		g_start = true;
	}
...
void Motion(int x, int y)
{
	...
	{
		y = 600 - y;
		g_target->SetTransform({float(x),float(y),0});
	}
}
